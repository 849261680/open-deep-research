...
class ResearchChains:
    ...
    def create_search_analysis_chain(self):
        """创建搜索结果分析链"""
    ...
    def create_report_generation_chain(self):
        """创建报告生成链"""
    ...
    def process_search_results(self, results: Dict[str, List[Dict[str, Any]]]):
        """处理搜索结果为文档"""
    ...
    async def analyze_search_results_with_chain(self, step_title: str, search_results: Dict[str, List[Dict[str, Any]]]) -> str:
        """使用链分析搜索结果"""
        return await analysis_chains.analyze_search_results_with_chain(step_title, search_results)
